...
void FatherDo(struct dirent *entry, struct stat statbuf, char *full_path)
{
...
void dealDir(const char* path)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if(!S_ISDIR(statbuf.st_mode))
        {
            ...
            if(subpid == 0)  // 子进程 
            {
                execl("./mycp","mycp",full_path,"/home/dh/labLinux/test",nullptr);
            }
            else  // 父进程
            {
                int status;
                pid_t ret = waitpid(subpid, &status, 0);
                // 父进程要做的事情
                FatherDo(entry,statbuf,full_path);
            }
        }
        else {
            // 是目录的处理
